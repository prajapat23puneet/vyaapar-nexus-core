    ...
    IRequestHandler<CancelOrderCommand, bool>
{
    ...
    public async Task<CreateOrderResponse> Handle(CreateOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        VyaaparNexus.Application.Observability.MetricsRegistry.OrdersSubmittedTotal.Inc();
    ...
    public async Task<bool> Handle(CancelOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        var saga = await _context.SagaStates.FirstOrDefaultAsync(s => s.OrderId == request.OrderId, cancellationToken);

        if (order == null || saga == null)
            return false;

        if (order.Status != OrderStatus.Submitted && order.Status != OrderStatus.InventoryChecking)
            throw new InvalidOperationException($"Cannot cancel order in status: {order.Status}");

        var now = DateTimeOffset.UtcNow;
        var reason = "Cancelled by user request";
